...
def test_score_stage_surfaces_per_job_failures(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        scorer,
        "run_scoring",
        lambda **kwargs: {"scored": 2, "errors": 1, "elapsed": 0.1, "distribution": []},
    )
...
def test_targeted_dry_run_records_job_url(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    conn = database.init_db(tmp_path / "openapplypilot.db")
    job_url = "https://example.test/target"
    conn.execute(
        "INSERT INTO jobs (url, title, site, full_description) VALUES (?, 'Target', 'Example', 'JD')",
        (job_url,),
    )
    conn.commit()
    monkeypatch.setattr(pipeline, "load_env", lambda: None)
    ...
    monkeypatch.setattr(
        ...
        "record_stage_event",
        lambda run_id, stage, status, **kwargs: database.record_stage_event(
            run_id, stage, status, conn=conn, **kwargs
        ),
    )
    monkeypatch.setattr(
        pipeline,
        "finish_pipeline_run",
        lambda run_id, status, **kwargs: database.finish_pipeline_run(
            run_id, status, conn=conn, **kwargs
        ),
    )
    ...
    events = conn.execute(
        "SELECT stage, job_url FROM stage_events WHERE run_id = ? ORDER BY id",
        (result["run_id"],),
    ).fetchall()
